...
SECURITY_ENV.md Environment variable security & handling guide
```

Additional documentation files (all in repository root – no `docs/` folder):

| File | Summary |
|------|---------|
| `RUNBOOK_RPC_OUTAGE.md` | Steps when primary RPC fails; switch fallback / rotate provider key. |
| `RUNBOOK_INGESTION_RETRY.md` | How to force re-ingestion of missed on-chain events (block range replay). |
| `RUNBOOK_QUEUE_CLEANUP.md` | Cleaning Redis/Celery queues & anomaly dedup keys safely. |
| `RUNBOOK_ENV_RESET.md` | Full local environment reset (containers, volumes, caches, migrations). |
| `CHECKLIST_PRE_RELEASE.md` | Verification before tagging a release (tests, migrations, gas, ABI diff). |
| `CHECKLIST_POST_RELEASE.md` | Post-deploy validation & monitoring handoff. |
| `backend/README.md` | API, ingestion process, configuration, troubleshooting. |
| `contracts/README.md` | Contract architecture, upgrade (UUPS), ABI regeneration, testing. |
...
Only list what must be acquired or generated securely outside the repo (details are already documented in each `.env.example`).
